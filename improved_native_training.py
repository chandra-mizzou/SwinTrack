...
# --- IMPROVED NATIVE RESOLUTION TRAINING ---
num_epochs = 100
MAX_SIZE = 512  # Reduced for better training stability
...
class ImprovedNativeDataset(Dataset):
    def __init__(self, folder1, folder2, folder3, filenames, is_training=True, max_size=512):
...
# Create datasets
train_dataset = ImprovedNativeDataset(folder1, folder2, folder3, train_files, is_training=True, max_size=MAX_SIZE)
val_dataset = ImprovedNativeDataset(folder1, folder2, folder3, val_files, is_training=False, max_size=MAX_SIZE)
test_dataset = ImprovedNativeDataset(folder1, folder2, folder3, test_files, is_training=False, max_size=MAX_SIZE)
...
# IMPROVED Content Preservation Loss
class ImprovedContentLoss(nn.Module):
    ...
    def forward(self, fused, rgb, thermal, gt):
        ...
        # TOTAL LOSS - Rebalanced for better learning
        total_loss = (1.0 * gt_loss + 
                     0.6 * rgb_content_loss +
                     0.6 * thermal_content_loss +
                     0.2 * perceptual_loss +
                     0.1 * gradient_loss)
...
def improved_train(model, train_loader, val_loader, test_dataset, epochs, device='cpu'):
    model = model.to(device)
    criterion = ImprovedContentLoss(device=device)
    ...
    # IMPROVED Learning rate scheduler - less aggressive
    ...
    scaler = GradScaler()
    best_loss = float('inf')
    early_stop_patience = 25  # Increased patience
    ...
    for epoch in range(epochs):
        ...
        if epoch < warmup_epochs:
            lr = 5e-4 * (warmup_factor + (1 - warmup_factor) * epoch / warmup_epochs)
        ...
        if val_loss < best_loss - 1e-6:
            ...
            torch.save(model.state_dict(),
                '/usr/mvl2/cvrfr/KeyPonFuse_V1/Codes/models_22aug25/model_training/trained_models/UNet_improved_native_resolution.pth')
        else:
...
train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True, collate_fn=native_collate, num_workers=2)
val_loader = DataLoader(val_dataset, batch_size=4, shuffle=False, collate_fn=native_collate, num_workers=2)

print("🚀 Starting IMPROVED Native Resolution Training...")
print(f"📊 Max image size: {MAX_SIZE}px (reduced for stability)")
print(f"📊 Batch size: 4 (increased for better gradients)")
print(f"📊 Focus: Retain content from both RGB and Thermal")
print(f"📊 Reference: Learn from GT fusion output")
print(f"📊 Loss: L1 + SSIM + Perceptual + Content Preservation + Gradient")
print(f"📊 Optimizer: AdamW with better LR scheduling")
print(f"📊 Learning Rate: 5e-4 with warmup and ReduceLROnPlateau")

improved_train(model, train_loader, val_loader, test_dataset, epochs=num_epochs, device=device)
