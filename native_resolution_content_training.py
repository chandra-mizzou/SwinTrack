...
# --- NATIVE RESOLUTION CONTENT PRESERVATION TRAINING ---
num_epochs = 75
...
# NATIVE RESOLUTION Dataset - NO resizing
class NativeResolutionDataset(Dataset):
    def __init__(self, folder1, folder2, folder3, filenames, is_training=True, max_size=1024):
        ...
        if is_training:
            self.img_transform = transforms.Compose([
                transforms.Lambda(self.resize_if_needed),  # Only resize if too large
    ...
    def resize_if_needed(self, img):
        """Only resize if image is larger than max_size to prevent OOM"""
        w, h = img.size
    ...
    def __getitem__(self, idx):
        ...
        # Apply transforms (minimal processing)
...
def native_collate(batch):
    """Native resolution collate function - handles variable sizes with padding"""
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss - 1e-6:
            ...
            torch.save(model.state_dict(),
                '/usr/mvl2/cvrfr/KeyPonFuse_V1/Codes/models_22aug25/model_training/trained_models/UNet_native_content_preservation.pth')
        else:
...
# NATIVE RESOLUTION: Small batch size for memory efficiency
...
print("🚀 Starting Native Resolution Content Preservation Training...")
print(f"📊 Image size: Native resolution (up to 1024px)")
