...
# NATIVE RESOLUTION Dataset class
class NativeResolutionDataset(Dataset):
    def __init__(self, folder1, folder2, folder3, filenames, is_training=True, max_size=1024):
        ...
        self.max_size = max_size  # Maximum size to avoid memory issues

        # NATIVE RESOLUTION: Only resize if image is too large
        if is_training:
            self.img_transform = transforms.Compose([
                transforms.Lambda(self.resize_if_needed),  # Custom resize function
...
# Create datasets with native resolution
train_dataset = NativeResolutionDataset(folder1, folder2, folder3, train_files, is_training=True, max_size=1024)
val_dataset = NativeResolutionDataset(folder1, folder2, folder3, val_files, is_training=False, max_size=1024)
test_dataset = NativeResolutionDataset(folder1, folder2, folder3, test_files, is_training=False, max_size=1024)

def custom_collate(batch):
    """Custom collate function to handle variable sizes"""
...
# IMPROVED Loss Function for Native Resolution
class NativeResolutionLoss(nn.Module):
    def __init__(self, device='cpu', 
                 lambda_gt=2.0,
                 lambda_perceptual=0.3,  # Reduced for memory
                 lambda_structural=1.0,
                 lambda_modality=0.8,
                 lambda_gradient=0.3,
                 lambda_rgb=0.5, 
                 lambda_thermal=0.5):
        super(NativeResolutionLoss, self).__init__()
        ...
        # VGG for perceptual loss (use fewer layers for memory)
        vgg = vgg19(pretrained=True)
        self.vgg_features = nn.Sequential(*list(vgg.features)[:16]).eval().to(device)  # Fewer layers
    ...
    def forward(self, fused, rgb, thermal, gt):
        ...
        # Perceptual loss (downsample for memory efficiency)
        if fused.size(1) == 1:
        ...
        # Downsample for VGG to save memory
        if fused_3ch.shape[-1] > 512:
            scale_factor = 512 / fused_3ch.shape[-1]
        ...
        if fused.shape[-1] > 512:
            scale_factor = 512 / fused.shape[-1]
        ...
        structural_loss = 1 - self.ms_ssim(fused_down, gt_down)
    ...
    def ms_ssim(self, pred, target, levels=3):  # Reduced levels for memory
...
# NATIVE RESOLUTION Training Function
def native_resolution_train(model, train_loader, val_loader, test_dataset, epochs, device='cpu'):
    model = model.to(device)
    criterion = NativeResolutionLoss(device=device)
    
    # Optimizer with gradient accumulation for large images
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss - 1e-6:
            ...
            torch.save(model.state_dict(),
                '/usr/mvl2/cvrfr/KeyPonFuse_V1/Codes/models_22aug25/model_training/trained_models/UNet_native_resolution.pth')
        else:
...
# NATIVE RESOLUTION: Smaller batch size for memory
train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, collate_fn=custom_collate, num_workers=2)  # Smaller batch
val_loader = DataLoader(val_dataset, batch_size=2, shuffle=False, collate_fn=custom_collate, num_workers=2)

print("🚀 Starting Native Resolution Training...")
print(f"📊 Max image size: 1024px")
