...
def main():
	...
	selected_labels = []  # Changed to list for multiple targets
	...
	while frame_idx < len(image_paths):
		...
		if not paused:
			...
			if track_only_selected and selected_labels and not selected_lost:
				outputs = [(b, lbl, traj) for (b, lbl, traj) in outputs if lbl in selected_labels]
			elif track_only_selected and selected_lost:
				print("[INFO] Selected target(s) lost. Resuming all targets.")
				track_only_selected = False
				selected_labels = []
			...
			for box, lbl, traj in outputs:
				b = list(map(int, box))
				color = DRAW_SELECTED_COLOR if lbl in selected_labels else DRAW_GENERAL_COLOR
