...
# --- ROBUST NATIVE RESOLUTION TRAINING ---
num_epochs = 75
...
# ROBUST Dataset - Force all images to same size to avoid issues
...
# Create datasets
train_dataset = RobustDataset(folder1, folder2, folder3, train_files, is_training=True, target_size=512)
val_dataset = RobustDataset(folder1, folder2, folder3, val_files, is_training=False, target_size=512)
test_dataset = RobustDataset(folder1, folder2, folder3, test_files, is_training=False, target_size=512)
...
# ROBUST Content Preservation Loss
class RobustContentLoss(nn.Module):
    def __init__(self, device='cpu'):
        super(RobustContentLoss, self).__init__()
        self.device = device
...
def robust_train(model, train_loader, val_loader, test_dataset, epochs, device='cpu'):
    model = model.to(device)
    criterion = RobustContentLoss(device=device)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_loss - 1e-6:
            ...
            torch.save(model.state_dict(),
                '/usr/mvl2/cvrfr/KeyPonFuse_V1/Codes/models_22aug25/model_training/trained_models/UNet_robust_content_preservation.pth')
        else:
...
# ROBUST: Fixed size training to avoid size mismatch issues
...
print("🚀 Starting Robust Content Preservation Training...")
print(f"📊 Image size: 512x512 (fixed for stability)")
...
print(f"📊 Fixed size to avoid dimension mismatch issues!")

robust_train(model, train_loader, val_loader, test_dataset, epochs=num_epochs, device=device)
