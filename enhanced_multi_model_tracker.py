...
class EnhancedCommandProcessor:
	def __init__(self, tokenizer, bert_model, device):
		...
		self.patterns = {
			'multiple_targets': r'track\s+([a-z])\s+and\s+([a-z])',
			'spatial_relationship': r'track\s+target\s+(left|right|above|below|near|far)\s+of\s+([a-z])',
			'object_with_property': r'track\s+target\s+with\s+([a-z\s]+)',
			'switch_track': r'switch\s+track\s+to\s+target\s+(left|right|above|below|near|far)\s+of\s+([a-z])',
	...
	def parse_command(self, command, current_outputs, tracker):
		...
		for box, label, traj in current_outputs:
			current_targets[label] = {
				...
				'class_name': class_id_to_name(tracker.registry[label]['class_id']) if label in tracker.registry else 'unknown'
			}
	...
	def _execute_pattern(self, pattern_name, match, current_targets, command):
		...
		if pattern_name == 'multiple_targets':
			...
			labels = [f"{label1}_1", f"{label2}_1"]  # Simplified for demo
			return self._find_best_matches(labels, current_targets)
			...
			reference_label = f"{reference_label}_1"  # Simplified
	...
	def _find_best_matches(self, target_labels, current_targets):
		"""Find best matches for multiple target labels"""
		matches = []
		for label in target_labels:
			best_match = None
			best_sim = -1.0
			for current_label, props in current_targets.items():
		...
		return matches, sum(sim for _, sim in matches) / len(matches) if matches else 0.0
...
class TrackRegistry:
    def __init__(self, center_reuse_px=CENTER_REUSE_PX, miss_tol=MISS_TOLERANCE_FRAMES, history_frames=HISTORY_FRAMES):
        ...
        self.registry = {}  # label -> state dict {base, last_center, last_frame, missing, trajectory, last_velocity}
...
def main():
	...
	while frame_idx < len(image_paths):
		...
		if key == ord('q'):
			...
			print("  - track a and b (track multiple targets)")
			print("  - track target left/right/above/below of target_a")
			print("  - track target with ball")
			print("  - switch track to target left/right of target_a")
